...
def fetch_and_clean():
    ...
    open(f"{prefix}cn_tw.txt", "w").close()
    open(f"{prefix}tv_v4.txt", "w").close()
    ...
    for name, url in sources.items():
        ...
        try:
            ...
            for channel_name, link in matches:
                ...
                elif any(x in name_u for x in ["香港", "HK", "翡翠", "TVB", "凤凰", "澳门", "MO"]): group = "港澳电视"
                elif any(x in name_u for x in ["台湾", "TW", "中天", "年代", "TVBS"]): group = "台湾电视"
                else:
                    # 地市识别逻辑
                ...
                if group != "其他":
                    all_channels.append({"name": channel_name.strip(), "url": link, "group": group, "v6": is_ipv6})
                    seen_urls.add(link)
        except Exception as e: print(f"{name} 失败: {e}")

    # 分组排序字典
    group_order = {"中央台": 0, "卫视": 1, "港澳电视": 2, "台湾电视": 3, "北京频道": 4, "上海频道": 5, "广东频道": 6}

    # 排序：权重 -> 组名 -> 频道名 -> IPv4优先
    all_channels.sort(key=lambda x: (group_order.get(x['group'], 99), x['group'], x['name'], 1 if x['v6'] else 0))

    # 写入文件
    with open(f"{prefix}cn_tw.txt", "w", encoding="utf-8") as f_all, \
         open(f"{prefix}tv_v4.txt", "w", encoding="utf-8") as f_v4:
        curr_all, curr_v4 = None, None
        for c in all_channels:
            if c['group'] != curr_all:
                curr_all = c['group']
                f_all.write(f"{curr_all},#genre#\n")
            f_all.write(f"{c['name']},{c['url']}\n")
            
            if not c['v6']:
                if c['group'] != curr_v4:
                    curr_v4 = c['group']
                    f_v4.write(f"{curr_v4},#genre#\n")
                f_v4.write(f"{c['name']},{c['url']}\n")
    print(f"处理完成！总频道：{len(all_channels)}")
